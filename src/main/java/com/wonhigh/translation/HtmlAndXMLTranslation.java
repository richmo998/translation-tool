 ...
 * @description:
 */
public class HtmlAndXMLTranslation extends BaseTranslation {
    private static Logger logger = Logger.getLogger(HtmlAndXMLTranslation.class);
    ...
    public List<String> translate(List<String> fileList, String language) {
        ...
        for (String line : fileList) {
            ...
            //忽略html/htm/xml相关注释
            //1.注释单独成行；2.代码中嵌入部分注释
            if (line.contains(Constants.ANNOTATION_HTML_XML_START)
                    && line.contains(Constants.ANNOTATION_HTML_XML_END)) {
                //单行全为注释
                translationList.add(line);
                continue;

            } else if (line.contains(Constants.ANNOTATION_HTML_XML_START)
                    && !line.contains(Constants.ANNOTATION_HTML_XML_END)) {
                //多行注释开始
                translationList.add(line);
